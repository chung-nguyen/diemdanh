...
const ImportForm: React.FC<ImportFormProps> = ({ onCancel, onDone, visible }) => {
  const intl = useIntl();
  ...
  const props: UploadProps = {
    name: 'file',
    multiple: false,
    action: getAPIURL('meeting/import'),
    headers: {
  ...
  return (
    <Modal open={visible} onOk={onCancel} onCancel={onCancel}>
      <Dragger {...props}>
        ...
        <p className="ant-upload-hint">Tạo hội nghị mới kèm khách mời bằng cách nhập file Excel.</p>
      </Dragger>
    </Modal>
  );
};
